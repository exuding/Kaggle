...
def BinBadRate(df, col, target, grantRateIndicator=0):
    ...
    B = sum(regroup['bad'])
    overallRate = B * 1.0 / N
    return (N, dicts, regroup, overallRate)
...
def ChiMerge(df, col, target, max_interval=6, special_attribute=[], minBinPcnt=0):
    '''
    ...
    if N_distinct <= max_interval:  # 如果原始属性的取值个数低于max_interval，不执行这段函数
        ...
        if minBinPcnt > 0:
            ...
            while minPcnt < minBinPcnt and len(cutOffPoints) > 1:
                # 找出占比最小的箱
                ...
                valueCounts = valueCounts.sort_index()
                minPcnt = min(valueCounts['pcnt'])
        if len(special_attribute) > 0 and special_attribute[0] in cutOffPoints:
        ...
        (N, binBadRate, regroup, overallRate) = BinBadRate(df2, 'temp', target, grantRateIndicator=1)
...
def AssignBin_Sort(x, cutOffPoints, special_attribute=[]):
    '''
    :param x: 某个变量的某个取值
    :param cutOffPoints: 上述变量的分箱结果，用切分点表示
    :param special_attribute:  不参与分箱的特殊取值
    :return: 分箱后的对应的第几个箱，从0开始
    for example, if cutOffPoints = [10,20,30], if x = 7, return Bin 0. If x = 35, return Bin 3，左开右闭
    '''
