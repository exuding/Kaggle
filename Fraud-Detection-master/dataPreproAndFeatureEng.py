...
if __name__=='__main__':
    ...
    for feature in ['id_02__id_20', 'id_02__D8', 'D11__DeviceInfo', 'DeviceInfo__P_emaildomain',
                    ...
                    'id_01__card1', 'id_01__card3', 'card3__card5', 'ProductCD__TransactionAmt', 'card1__dist1',
                    'card1__P_emaildomain', 'card4__dist1', 'addr1__card4', 'P_emaildomain__addr1']:
    ...
    # 训练集和测试集分布不一样的特征
    removeFeature = ['DT_M', 'DT_W', 'DT_D', 'DT_M_total', 'DT_W_total', 'Cgroup_nan_sum']
    train.drop(removeFeature, axis=1, inplace=True)
    test.drop(removeFeature, axis=1, inplace=True)
    ...
    if not os.path.isfile(pickle_file):  # 判断是否存在此文件，若无则存储
        ...
        try:
            with open('trainAndTestData0925.pickle', 'wb') as pfile:
                pickle.dump(
                    {
