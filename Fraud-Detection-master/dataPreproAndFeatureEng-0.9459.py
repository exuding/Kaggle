...
if __name__=='__main__':
    ...
    # 其他衍生变量 mean 和 std
    for df in [train, test]:
        for item in ['card1', 'card4','C13','D10']:
    ...
    for df in [train, test]:
        for item in ['card1', 'card4','addr1','D10']:
    ...
    for df in [train,test]:
        for item in ['card1','card2','card3','card4','card5','C13','D10','addr1','D1','D2','C1','D4']:
    ...
    for feature in ['card1', 'card2', 'card3', 'card4', 'card5', 'card6', 'id_34', 'id_36','C13']:
        ...
        test[feature + '_count_full'] = test[feature].map(
            pd.concat([train[feature], test[feature]], ignore_index=True).value_counts(dropna=False))
    # D的 #变量用count做衍生  计数编码
    for feature in  ['D1','D2','D3','D4','D5','D6','D8','D10','D11','D12','D13','D14','D15']:
        train[feature + '_count_full'] = train[feature].map(
            pd.concat([train[feature], test[feature]], ignore_index=True).value_counts(dropna=False))
        test[feature + '_count_full'] = test[feature].map(
            pd.concat([train[feature], test[feature]], ignore_index=True).value_counts(dropna=False))
    ...
    #C13/C6	C13/C2  C13/C11  C2/C11  C13/C1  C13*TransactionAmt
    #缺失值的填充
    for df in [train, test]:
    ...
    #排列组合交叉生成
    cross_features = list(combinations(serialFeatureTop, 2))
    for df in [train, test]:
